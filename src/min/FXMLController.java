...
public class FXMLController {
            ...
            if (change.next()) {
                if (change.getAddedSize() > 0) {
                    for (Block block : change.getAddedSubList()) {
                        ...
                        loader.setLocation(ClassLoader.getSystemResource("main/resources/fxml/BlockInfo.fxml"));
                        loader.setController(new FXMLBlockController(block, "Block " + counter++, blockChain, chain));
                        try {
                            GridPane pane = loader.load();
                            pane.getStyleClass().add("grid-pane");
                            pane.getStylesheets().add(ClassLoader.getSystemResource("main/resources/css/valid-block.css").toExternalForm());
        ...
        if (result.isValid()) {
            ...
            for (int i = result.getInvalidPos(); i < chain.getChildren().size(); i++) {
                ...
                if (node instanceof GridPane) {
                    ...
                    ((GridPane) node).getStylesheets().add(ClassLoader.getSystemResource("main/resources/css/invalid-block.css").toExternalForm());
                }
            }
        }
    }
    ...
    private void resetChainColor() {
        chain.getChildren().forEach(node -> {
            if (node instanceof GridPane) {
                ((GridPane) node).getStylesheets().clear();
                ((GridPane) node).getStylesheets().add(ClassLoader.getSystemResource("main/resources/css/valid-block.css").toExternalForm());
            }
        });
    }
